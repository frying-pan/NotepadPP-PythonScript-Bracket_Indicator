...
s_option_style = s_option_style.upper().strip() # upper-case, without spaces around indicator style
...
class C_BracketIndicator_CB():
	...
	def Ugly_ClearAllTabsHighlights(self, i_indicator_number):
		def _Clear_All_Tabs(i_indicator_number):
		...
		if notepad.isSingleView():
			i_act_index = notepad.getCurrentDocIndex(i_act_view)
			_Clear_All_Tabs(i_indicator_number)
			notepad.activateIndex(i_act_view, i_act_index)
		else:
			i_act_index_0	= notepad.getCurrentDocIndex(0)
			i_act_index_1	= notepad.getCurrentDocIndex(1)
	...
	def RegCallback(self):
		...
		def _CheckBrackets():
			def _Clear_Whole_Doc_Rem_Range(curedit):
			...
			def _MatchingBrackets_OutRange_InRange(curedit, i_start, i_end, i_min, i_max, b_angle, b_sl_angle):
					...
					for o_match in pattern_brackets.finditer(s_text):
						...
						if dic_charstyle_datas[s_charstyle_bk][c_bk] == i_open:
							if s_charstyle_bk == s_charstyle_fw:
								...
								# remember bk pos (un-matched opening before selection) for future forward match
				...
				while True: # forward search from (first un-matched)i_pos_fw + 1, i_offset >= 0, i_chunk > 0
					...
					for o_match in pattern_brackets.finditer(s_text):
						...
						# un-matched closing charstyle found, look for a previously remembered bk pos or kill it
						if dic_charstyle_datas[s_charstyle_fw][c_fw] == i_close:
			...
			if bufferid_cur != self.bufferid_last:
				...
				_Clear_Whole_Doc_Rem_Range(curedit)

			t_ranges = _Get_Search_Ranges(curedit)
			if t_ranges is None: # visible area is outside highlight limits : no highlight needed
				if self.had_range: _Clear_Whole_Doc_Rem_Range(curedit)
				return
			...
			if t_brackets is None: # no brackets found around selection (within highlight limits)
				if self.had_range: _Clear_Whole_Doc_Rem_Range(curedit)
				return

			i_start	= t_brackets[0]
			i_end	= t_brackets[1]
			if not(self.b_brackets): i_start += 1
			elif self.b_content: i_end += 1

			if self.had_range: # can skip (same highlight(s)), or clear last highlight range(s) for new one(s)
				if _CanSkip_Or_ClearLastRange(curedit, i_start, i_end, self.b_content):
					return

			if self.b_content: # with content, with or without brackets (highlight 1 range)
				if i_end > i_start:
					curedit.setIndicatorCurrent(self.indicator_number)
					...
					self.had_range = False
			elif self.b_brackets: # brackets only (highlight 2 ranges of 1 char long)
				curedit.setIndicatorCurrent(self.indicator_number)
...
def Main():
	...
	if console.editor.getProperty(s_editorprop_cb_on) != str(i_true):
		...
		o_bracketindicator_cb.Ugly_ClearAllTabsHighlights(int(console.editor.getProperty(s_editorprop_indic_num)))
