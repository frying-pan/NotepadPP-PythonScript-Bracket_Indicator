...
i_option_highlight_angle	= 0	# set to INTEGER 1 to enable option : also highlight <> angle brackets (in addition to () [] {})
...
t_option_key = ("S_STYLE", "COL_RGB", "ALPHA", "OUTALPHA", "BRACKETS", "CONTENT", "LIMIT_HL", "ANGLE", "TOGMBOX")
t_option_value = ( \
	s_option_style.upper(), t_option_rgb_color, i_option_alpha, i_option_outline_alpha, \
	i_option_highlight_brackets, i_option_highlight_content, i_option_limit_highlight, i_option_highlight_angle, i_option_toggle_msgbox)
t_option_initdes = ( \
	"Box style", "(Red,Green,Blue)", "Alpha", "Ouline alpha", \
	"Brackets", "Content", "Limit Highlight", "<> Brackets", "Toggle message box")
...
class C_BracketIndicator_CB():
	...
	def __init__(self, s_script_name, dic_styles, i_option_on, s_editorprop_cb_on, dic_editorprop):
		from FP__Lib_Edit import C_Find_OutRange
		self.o_find_outrange = C_Find_OutRange()

		# local const
		self.INVALID_POSITION = -1 # Scintilla character position in line constant
	...
	def Ugly_ClearAllBuffersHighlights(self, i_indicator_number):
		...
		for t_file_infos in lst_files_infos:
			...
			editor.setIndicatorCurrent(i_indicator_number)
	...
	def RegCallback(self):
		...
		def CB_BracketInd_BufferActivated(args):
			if self.last_buffid == notepad.getCurrentBufferID():
				CheckBrackets()

		def CheckBrackets():
			def CanSkip_Or_ClearLastRange(i_start, i_end, b_content):
				if b_content:
					i_cur_start	= editor.indicatorEnd(self.indicator_number, 0)
					i_cur_end	= editor.indicatorEnd(self.indicator_number, i_cur_start)
					...
					if i_cur_end != 0:
						...
						editor.indicatorClearRange(i_cur_start, i_cur_end - i_cur_start)
					else:
					...
					i_cur_start		= editor.indicatorEnd(self.indicator_number, 0)
					i_cur_end		= editor.indicatorEnd(self.indicator_number, i_cur_start)
					i_cur_start2	= editor.indicatorEnd(self.indicator_number, i_cur_end)
					i_cur_end2		= editor.indicatorEnd(self.indicator_number, i_cur_start2)
					...
					if i_cur_end != 0:
						...
						editor.setIndicatorCurrent(self.indicator_number)
						editor.indicatorClearRange(i_cur_start, i_cur_end2 - i_cur_start)
					else:
			...
			i_lines_on_screen = editor.linesOnScreen()
			...
			i_max = editor.getLineEndPosition(editor.docLineFromVisible(i_vis_line_last))
			...
			t_brackets = self.o_find_outrange.RE_MatchingBrackets_OutRange_InRange( \
							editor, editor.getSelectionStart(), editor.getSelectionEnd(), i_min, i_max, b_angle)
			...
			if (b_limit_hl and b_content):
				...
				i_cur_pos = editor.getCurrentPos()
				...
				if i_end > i_start:
					editor.setIndicatorCurrent(self.indicator_number)
					editor.indicatorFillRange(i_start, i_end - i_start)
					self.had_range = True
				else:
					self.had_range = False
			# NOT optimized for very long brackets content : with content (with or without brackets)
				...
				if self.had_range: # return, or clear last highlight range for update
					if CanSkip_Or_ClearLastRange(i_start, i_end, b_content):
						return
				if i_end > i_start:
					editor.setIndicatorCurrent(self.indicator_number)
					editor.indicatorFillRange(i_start, i_end - i_start)
			...
			# NO content, brackets only (highlight 2 ranges of 1 char long)
				...
				if self.had_range: # return, or clear last 2 highlight ranges for update
					if CanSkip_Or_ClearLastRange(i_start, i_end, b_content):
						return
				editor.setIndicatorCurrent(self.indicator_number)
				editor.indicatorFillRange(i_start, 1)
				editor.indicatorFillRange(i_end, 1)
				self.had_range = True
		...
		# install callback 1, should be sync to ensure it is fired first (avoids constant UPDATEUI when a doc is cloned)
		...
		# install callback 3, only to synchronize highlight of documents cloned and active in both views
